...
| `ASPIRERADIUS092` | Publish | A `ConfigureRadiusInfrastructure` callback set `RadiusSecuritySecretConstruct.Kind` to a kind whose Radius-required data keys are not all present — `basicAuthentication` requires `username`/`password`, `certificate` requires `tls.crt`/`tls.key`, `azureWorkloadIdentity` requires `clientId`/`tenantId`, and `awsIRSA` requires `roleARN`. Radius does not reject the missing fields directly; the secrets recipe surfaces them as an invalid Kubernetes object name at deploy time. Add the missing entries or use kind `generic`. Expression-valued and unrecognized kinds are not checked. |
