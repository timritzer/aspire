...
| Password, for Redis | Nothing. The pinned `kube-recipes/rediscaches` recipe deploys Redis **without authentication** and publishes no password secret, so consumers receive an empty password and a warning names the discarded value (`ASPIRERADIUS075`) |
...
| `ASPIRERADIUS075` | Publish (warning) | The recipe a backing resource maps to provisions no credential (today: Redis, whose `kube-recipes/rediscaches` recipe deploys an unauthenticated server). Consumers receive an empty password, and any password configured on the resource is not applied to the deployed workload. |
...
| `ASPIRERADIUS081` | Publish | A TLS-enabled backing endpoint was asked for `Scheme`, `TlsEnabled`, or `Url`. The mapped Radius type publishes no transport-security output, so the value would describe how the resource runs locally rather than how the recipe deploys it. Raised as `RadiusBackingResourceEndpointException`. |
